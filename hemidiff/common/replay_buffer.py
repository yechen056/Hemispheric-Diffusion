...
class ReplayBuffer:
    ...
    @property
    def n_cam_steps(self):
        # get the minimum n_steps across all cameras
        # n_steps = np.inf
        # for cam_ep_ends in self.cam_episode_ends:
        #     if cam_ep_ends is not None and len(cam_ep_ends) > 0:
        #         n_steps = min(n_steps, cam_ep_ends[-1])
                
        n_steps = min((ends[-1] for ends in self.cam_episode_ends if ends), default=np.inf)
        return 0 if n_steps == np.inf else n_steps
    ...
    @property
    def n_cam_episodes(self):
        # n_episodes = 0

        # for cam_ep_ends in self.cam_episode_ends:
        #     if len(cam_ep_ends) > n_episodes:
        #         n_episodes = len(cam_ep_ends)
        # return n_episodes
            
            # # If there's exactly one episode list, return its length
            # if len(self.cam_episode_ends) == 1:
            #     return len(self.cam_episode_ends[0])

            # # Use max() to find the longest episode list length, default to 0 if empty
        return max((len(cam_ep_ends) for cam_ep_ends in self.cam_episode_ends), default=0)
    ...
    def add_cam_episode(self, 
        ...
        for key, value in data.items():
            ...
            # da_arr = da.from_zarr(arr)
            # da_arr[curr_len:new_len] = value
            # da.to_zarr(da_arr, arr)
