...
class TrainPolicyWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # print(self.model.models[0].mid_modules[0].cond_encoder[1])
        ...
        # cuda kernel tuning
        # torch.backends.cudnn.benchmark = cfg.training.cuda_kernel_tuning
    ...
    def run(self):
        ...
        if cfg.training.resume:
            ...
            if lastest_ckpt_path.is_file():
                ...
                if self.epoch >= cfg.training.num_epochs:
                    print(f'Already trained for {self.epoch} epochs. Exiting.')
                    return
        ...
        # configure env
        lazy_eval = cfg.task.lazy_eval  # don't eval during training
        if not lazy_eval:
        ...
        with JsonLogger(log_path) as json_logger:
            while self.epoch < cfg.training.num_epochs:
                ...
                if (self.epoch % cfg.training.rollout_every) == 0:
                    ...
                    else:   # set success rate to monotonic increasing for latest k ckpts saving
                ...
                # run diffusion sampling on sample batch
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        for tag, sample_batch in [
                            ('train_action_mse', train_sampling_batch),
                            ('test_action_mse', test_sampling_batch)
                        ]:
                        ...
                        if 'batch' in locals():
                            del batch
                        if 'obs_dict' in locals():
                            del obs_dict
                        if 'gt_action' in locals():
                            del gt_action
                        if 'result' in locals():
                            del result
                        if 'pred_action' in locals():
                            del pred_action
                        if 'mse' in locals():
                            del mse
